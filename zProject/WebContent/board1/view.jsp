...
$(function() {
	...
	$("#btnList").click(function() {
		location.href="${path}/board1_servlet/list.do";
	});
	
	$("#edit").click(function() {
		if(document.getElementById('writer1').value!=document.getElementById('userid1').value){
			alert('권한이 없습니다.');
			exit;
		}
	...
	$("#btnReply").click(function(){
		document.form1.action="${path}/board1_servlet/reply.do";
		document.form1.submit();
	});
	
	$("#btnWrite").click(function(){
		location.href="${path}/board1/write.jsp";
	});
...
function comment_add(){
	...
	$.ajax({
		type: "post",
		url: "${path}/board1_servlet/comment_add.do",
		data: param,
		success: function(){
			$("#writer").val("");
			$("#content").val("");
...
function comment_list(){
	$.ajax({
		type: "post",
		url: "${path}/board1_servlet/commentList.do",
		data: "num=${dto.num}",
						...
						<tr>
							<td colspan="4">
								...
								<input type="button" value="글쓰기" id="btnWrite">
			      				...
			      				<form style="display: inline;" method="post" action="<%=context %>/board1_servlet/edit.do" id="form1" name="form1">
									<input type="hidden" name="num" value="${dto.num}">
									<input type="hidden" name="subject" value="${dto.subject}">
									<input type="hidden" name="content" value="${dto.content}">
									<input type="hidden" id="writer1" name="writer1" value="${dto.writer }">
									<input type="hidden" id="userid1" name="userid1" value="<%=userid %>">
				      				<button type="button" id="edit" name="edit">수정/삭제</button>
								</form>
								
								<form style="display: inline;" method="post" action="<%=context %>/board1_servlet/up.do" id="form2" name="form2">
									<input type="hidden" name="num" value="${dto.num}">
