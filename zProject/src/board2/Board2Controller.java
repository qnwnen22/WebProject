...
public class Board2Controller extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(url.indexOf("list.do")!=-1) {
			...
			Board2DTO dto=dao.viewReplace(num);
			request.setAttribute("dto", dto);

			String page="/board2/view.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			dao.insert(dto);
			String path="/board2_servlet/list.do";
			...
			dao.update(dto);
			String page="/board2_servlet/list.do";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			List<Board2CommentDTO> list=dao.commentList(num);
			request.setAttribute("list", list);
			//출력 페이지 이동
			String page="/board2/comment_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}else if(url.indexOf("comment_add.do")!=-1) {
			System.out.println("comment_add.do 실행");
			Board2CommentDTO dto=new Board2CommentDTO();
			//게시물 번호
			...
			List<Board2DTO> list=dao.searchList(search_option, keyword);
			request.setAttribute("list", list);
			request.setAttribute("search_option", search_option);
			request.setAttribute("keyword", keyword);
			String page="/board2/search.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			Board2DTO dto = dao.view(num);
			dto.setContent("===게시물의 내용===\n"+dto.getContent());
			request.setAttribute("dto", dto);
			request.setAttribute("subject", subject);
			String page="/board2/reply.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
