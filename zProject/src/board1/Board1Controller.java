...
public class Board1Controller extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(url.indexOf("list.do")!=-1) {
			...
			List<Board1DTO> list=dao.list(start,end);
			request.setAttribute("list", list);
			//페이지 네비게이션 출력을 위한 정보전달
			request.setAttribute("page", pager);
			
			String page="/main/board1.jsp";
			...
			Board1DTO dto=dao.viewReplace(num);
			request.setAttribute("dto", dto);

			String page="/board1/view.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			dao.insert(dto);
			String path="/board1_servlet/list.do";
			...
			dao.update(dto);
			String page="/board1_servlet/list.do";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			List<Board1CommentDTO> list=dao.commentList(num);
			request.setAttribute("list", list);
			//출력 페이지 이동
			String page="/board1/comment_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}else if(url.indexOf("comment_add.do")!=-1) {
			System.out.println("comment_add.do 실행");
			Board1CommentDTO dto=new Board1CommentDTO();
			//게시물 번호
			...
			List<Board1DTO> list=dao.searchList(search_option, keyword);
			request.setAttribute("list", list);
			request.setAttribute("search_option", search_option);
			request.setAttribute("keyword", keyword);
			String page="/board1/search.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			Board1DTO dto = dao.view(num);
			dto.setContent("===게시물의 내용===\n"+dto.getContent());
			request.setAttribute("dto", dto);
			request.setAttribute("subject", subject);
			String page="/board1/reply.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
